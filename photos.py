...
class employee_details:
    def __init__(self,root ):#constructor #root name of window
            ...
            if len(name) == 0:
                ...
                try:
                    if name.isnumeric():
                        ...
                        messagebox.showerror('Incorrect ID', msg)
            ...
            if len(name) == 0:
                ...
                try:
                    if re.match(regex,name):
                        ...
                        msg = 'Employee CNiC can\'t be Alphanumeric.'
                        messagebox.showerror('Incorrect CNIC', msg)
            ...
            if len(name) == 0:
                ...
                try:
                    if any(ch.isdigit() for ch in name):
                        ...
                        messagebox.showerror('Incorrect Employee Name', msg)
        ...
        #Employee Name Entry
        employee_name_entry=Entry(left_frame,textvariable=self.var_name,width=20,font=("time new roman",10,'italic','bold'),validate='focusout',validatecommand=name_validation)
        employee_name_entry.grid(row=1,column=1,padx=2,pady=10)
        #Employee dob label
        ...
        #Employee dob Entry
        employee_dob_entry=DateEntry(left_frame,textvariable=self.var_dob,width=17,font=("time new roman",10,'italic','bold'))
        ...
        def validate_contact():
            ...
            if mat:
                ...
                messagebox.showerror('Incorrect Contact', 'Write Correct Phone Number like 03157784521')
        ...
        def validate_email():
            ...
            if len(email) == 0:
                ...
                try:
                    if(re.fullmatch(regex, email)):
                        ...
                        messagebox.showerror('Incorrect Email Format', 'Write Email in Correct Format ')
            ...
            return True
        employee_email_entry=Entry(left_frame,textvariable=self.var_email,width=20,font=("time new roman",10,'italic','bold'))
        ...
        #salary Entry
        salary_entry=Entry(left_frame,textvariable=self.var_salary,width=20,font=("time new roman",10,'italic','bold'))
    ...
    def search_data(self):
        if self.var_search.get()=="" or self.var_searchTX.get()=="Select":
            ...
            try:
                ...
                sql = "SELECT employee_id,Name,CNIC,DOB,Phone_no,Email,Gender,Martial_Status,Department,Salary,Address,Job_Title FROM employee_data where employee_id='" +str(self.var_search.get()) + "'" 
                ...
                if len(rows)!=0:
                    ...
                    for i in rows:
                        self.employee_table.insert("",END,values=i)
                    if rows==None:
                        messagebox("Error","Data Not Found",self.root)
                        conn.commit()
                conn.close()
            except Exception as es:
                messagebox.showerror("Error",f"Due To :{str(es)}",parent=self.root)
    ...
    def update_data(self):
        if self.var_empid.get()=="" or self.var_name.get()=="" or self.var_cnic.get()=="" or self.var_dob.get()=="" or self.var_phone_no.get()=="" or self.var_email.get()=="" or self.var_gender.get()=="" or self.var_martial_status.get()=="" or self.var_job_title.get()=="" or self.var_department.get()=="" or self.var_salary.get()==0.0 or self.var_address.get()=="":
            ...
            try:
                ...
                if update >0:
                    conn = mysql.connector.connect(username='root', password='root',host='localhost',database='employee_management')
                    my_cursorr = conn.cursor()
                    
                    sql = "SELECT employee_id,Name,CNIC,DOB,Phone_no,Email,Gender,Martial_Status,Department,Salary,Address,Job_Title FROM employee_data where employee_id='" +self.var_empidq.get() + "'" 
                    my_cursorr.execute(sql)
                    
                    
                    # my_cursor.execute("select * from student where Roll_No= " +str(self.var_search.get())+" "+str(self.var_searchTX.get())+"")
                    rows=my_cursorr.fetchall()       
                    print("rows",len(rows))
                    print(rows)
                    con=mysql.connector.connect(host="localhost",username="root",password="root",database="employee_management")
                    my_cursor=con.cursor()                                                                                                                                                                                                                                                                                                                                                        
                ...
                def Crete_Folder_Images(name,id, tn,directory):
                    ...
                    for i in tqdm(range(1, 1+ tn)):
                        ...
                        for(x,y,w,h) in faces:
                            ...
                            cv.imwrite('Datasets/'+str(name)+'/'+str(name)+str(i)+'.jpg', res)
